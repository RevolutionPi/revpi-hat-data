...
def resolve_template(template_path: str) -> dict:
    """Return resolved dict from template and included file (if present)."""

    template_data = json.loads(Path(template_path).read_text())

    include_name = template_data.get("include")
    if include_name:
        include_path = Path("templates") / include_name
        include_data = json.loads(include_path.read_text())
        return include_data | template_data
...
for template in revpi_hat_templates:
    template_data = resolve_template(template)
    ...
    readme += f"| [{name }]({Path(template).as_posix()}) | {vendor} | PR{product_id} | R{product_revision:02} | {data_version} | {overlay} |\n"
